...
The pleasant surprise — and the reason the sequential operator is not a
transport-laden mess — is that every dependent type involved lines up
...
/-- **The sequential operator** `G ◁ F` ("`G` outside, `F` inside"). A shape is an
...
@[inherit_doc] scoped infixr:70 " ◁ " => Container.seq
...
lemma that pins down the convention for `◁`. -/
...
/-- The two halves are mutually inverse: `seqExtInv ∘ seqExt = id`. -/
theorem Container.seqExtInv_seqExt {G F : Container} {X : Type}
    (p : Ext (G ◁ F) X) : Container.seqExtInv (Container.seqExt p) = p := by
  rcases p with ⟨⟨t, f⟩, lab⟩
  rfl

/-- The two halves are mutually inverse: `seqExt ∘ seqExtInv = id`. This,
together with `seqExtInv_seqExt`, is the equation `⟦G ◁ F⟧ = ⟦G⟧ ∘ ⟦F⟧`. -/
theorem Container.seqExt_seqExtInv {G F : Container} {X : Type}
    (p : Ext G (Ext F X)) : Container.seqExt (Container.seqExtInv p) = p := by
  rcases p with ⟨t, m⟩
  rfl
...
state, are the comonoids of the sequential operator. The comonoid laws unfold to
