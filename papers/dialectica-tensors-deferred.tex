%% ---------------------------------------------------------------------------
%% The Dialectica tensors on Cont --- DEFERRED / HELD material
%%
%% Provenance: this is section 10 ("Beyond the census: the Dialectica tensors")
%% pulled OUT of papers/four-monoidal-chapter.tex on 2026-07-19, at Neil's
%% direction (email uid-65/68/69). The tensors lXtimes and rXtimes are NOT part
%% of the core three-chapter book programme; they are material for a later
%% chapter on Cont(C) for a base category C other than Set. Held here intact so
%% the census/monoidal chapter can close on the census and the (co)monoid table.
%%
%% Lead for the future chapter: the symmetric tensor lXtimes carries a
%% subgame-perfection game-theory flavour (both players respond adaptively to the
%% other's move) which is worth developing on its own terms in the Cont(C)
%% setting.
%%
%% TODO (supersession flag, do not silently ship): the closure discussion below
%% records "conjecturally, neither tensor is closed" and "the closure of rXtimes
%% is conjecturally negative". This is now SUPERSEDED. A later PROVE result
%% (proofs/2026-07-18-rtimes-left-closed.md; memory: dialectica-non-closure-one-sided)
%% shows rXtimes IS left-closed: it is DIRECTED-closed (left yes, right no), with
%% [q,r] = (Cont(q,r), (a,c) |-> S_q x coprod_t r[a t]). Update the "Structural
%% consequences" subsection before this file is ever promoted out of held status.
%%
%% CITATION TODO (carried over): the entries depaiva89, lnv2405, capucci2024 are
%% NOT deep-read from source; they are distinguishing-only and do not affect the
%% mathematics, but must be deep-read in a browse session before publication.
%% ---------------------------------------------------------------------------
...
\newcommand{\dtens}{\mathbin{\otimes_{\Dial}}}
...
\title{\textbf{The Dialectica tensors on $\Cont$}\\[0.3em]
  \large held material, deferred from the four-tensor chapter}
\author{MacBeth\\ \normalsize Kodamai AI Mathematician project}
\date{Held 19 July 2026 (content 17 July 2026)}

\begin{document}
\maketitle

\begin{abstract}
This is held material, deferred from the companion chapter \emph{Four canonical
monoidal structures on $\Cont$}. The four canonical structures ($+$, $\times$,
the Dirichlet tensor $\dir$, the sequential operator $\tri$) are the Day-family
census and belong to the core book. The two tensors studied here --- the
symmetric $\ltimes$ and the directed $\rtimes$ of Dorta--Jarvis--Niu --- lie
\emph{outside} the Day family, and are identified as the multiplicative and
directed Dialectica products of G\"odel's functional interpretation. They are
retained here for a later chapter on $\Cont(C)$ over a base $C\neq\Set$; see the
supersession note in the source header before promoting this file.
\end{abstract}

\section{Beyond the census: $\ltimes$ and $\rtimes$ as Dialectica products}
\label{sec:dialectica}

\subsection{Is there anything outside the Day family?}
\label{sec:dia-setup}

Theorem~A of the companion chapter accounts for the \emph{Day-convolutional}
part of the census. Recall its terms: a monoidal structure $(\odot,J)$ on $\Cont$ is
...
$(p\odot q)[(s,t)]=p[s]\star q[t]$ for a monoidal structure $(\star,I)$ on $\Set$
of which $\odot$ is the Day convolution. That theorem makes the Day
construction $(\star,I)\mapsto(\odot_\star,y^I)$ an \emph{equivalence} between
...
$+$ and $\tri$ sit outside for elementary reasons (the companion chapter's census).

An equivalence onto the \emph{convolutional} structures is, honestly read, a
statement about a sub-family. It leaves a sharper question open at the level of
$\Cont$ itself:
...
Dorta--Jarvis--Niu \cite{DJN23} and left \emph{uninterpreted}. This section
identifies them as the Dialectica tensors of Gödel's functional interpretation,
answering DJN's own stated open question --- the ``second instance'' of
\emph{why exactly these?} noted in the closest-neighbour discussion of the
companion chapter's provenance section.
...
$\Sigma\Pi\mathbf 1\simeq\Poly\simeq\Cont$ \cite[Prop.~2.9]{DJN23}. Beyond the
Day convolution $\dir$ and the sequential operator $\tri$, their \S6 exhibits
...
\prov{Cited: DJN \cite[\S6]{DJN23}, in container coordinates}
\end{Definition}
...
these monoidal products'' \cite{DJN23}.
...
the companion chapter's Day equivalence (its Theorem~A) does not reach them.
...
convolution, and the companion chapter's Theorem~A --- an equivalence onto the
\emph{convolutional} structures --- does not apply to them.
\end{proof}
...
\footnote{\textbf{Superseded, held for update.} The closure conjecture below is
now partly refuted: a later result (\texttt{proofs/2026-07-18-rtimes-left-closed.md})
shows $\rtimes$ \emph{is} left-closed --- it is \emph{directed}-closed (left yes,
right no), with $[q,r]_\rtimes=\bigl(\Cont(q,r),\,(a,c)\mapsto S_q\times
\coprod_t r[a\,t]\bigr)$. So the honest statement is that $\ltimes$ is bi-non-closed
while $\rtimes$ is left- but not right-closed. This section is retained
\emph{intact} pending its promotion out of held status; do not ship without
folding in this correction.}
\prov{MacBeth (computed); $\rtimes$ left-closed (proved, see footnote)}
\end{Proposition}
...
These are, to our knowledge, the first non-\emph{bi}closed monoidal structures in
the $\Cont$ story, in sharp contrast with the census, where $\times$ is cartesian
closed, $\dir$ is closed, and $\tri$ has a right coclosure (all in the companion
chapter). Because
$\ltimes/\rtimes$ are non-convolutional, this non-closure says nothing about
whether the \emph{convolutional} tensors are closed; the two questions are
disjoint.

\begin{Remark}[A lead on closure, not a refutation]
\label{rem:dia-closure}
The non-closure argument above is a \emph{left}-closure argument: it rules out a
right adjoint to $(-)\ltimes q$ because that functor does not even preserve
coproducts. It does not settle every reasonable sense of ``closed.''
Lucatelli~Nunes--Vákár \cite{lnv2405} equip Grothendieck constructions --- of
which $\Cont$ is one --- with a Dialectica-flavoured monoidal \emph{closed}
structure in which the Dialectica twist lives in the \emph{internal hom}
$\multimap$, not in the tensor (their tensor is an untwisted, product-like
tensor lifted from base and fibre). If a hom
with a $\ltimes$-shaped twist exists on $\Cont$ it will be found on their model,
not by adjointness to $\ltimes$; we flag this as a lead for a later pass rather
than a refutation of the conjecture recorded in
Proposition~\ref{prop:noncocont}.
\end{Remark}
...
theme of DJN \cite{DJN23} itself.
...
orthogonal to the identification made here (see Remark~\ref{rem:dia-closure}).
...
\begin{Remark}[Grade and novelty]
\label{rem:dia-grade}
...
we grade the mathematics \textsf{[computed / an identification]}, the meet with
de~Paiva's tensor formula, which we cite from the standard reference
\cite{depaiva89} rather than reproduce. Its \emph{novelty} was checked by a
dedicated literature sweep against the recent Dialectica--polynomial corpus
(de~Paiva, Trotta, Spivak, Hedges, Lucatelli~Nunes--Vákár, Capucci et al.,
2023--2026); the verdict was \emph{clear}. The claim is framed accordingly, and
this framing is the honest one: we identify a \emph{known} tensor --- de Paiva's
--- with DJN's uninterpreted operation $\ltimes$; we do \emph{not} claim to be
the first to relate Dialectica and $\Poly$, which would be false. The two nearest
neighbours are cited and distinguished above (Lucatelli~Nunes--Vákár
\cite{lnv2405}, Capucci et al.\ \cite{capucci2024}); neither produces $\ltimes$
nor answers DJN's question. What DJN, who know $\Dial(\Set)\simeq\Hmg(2)$
intimately, nonetheless posed as open, this section settles.
\end{Remark}
...
\bibitem{depaiva89} V.~de~Paiva.
  \emph{The Dialectica Categories}.
  In \emph{Categories in Computer Science and Logic} (Boulder, 1987),
  Contemporary Mathematics 92, American Mathematical Society, 1989, 47--62;
  see also Ph.D.\ thesis / Tech.\ Report 213, University of Cambridge Computer
  Laboratory, 1991.

\bibitem{lnv2405} F.~Lucatelli~Nunes, M.~V\'ak\'ar.
  \emph{Monoidal closure of Grothendieck constructions via
  $\Sigma$-tractable monoidal structures and Dialectica formulas}.
  arXiv:2405.07724, 2024; to appear in \emph{Theory and Applications of
  Categories}.

\bibitem{capucci2024} M.~Capucci, B.~Gavranovi\'c, A.~Malik, F.~Rios,
  J.~Weinberger.
