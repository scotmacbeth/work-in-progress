...
They transport the Day tensor $\dir$ up to $\Sigma\Pi\mathcal C$ --- where it
agrees with ours --- and construct a \emph{weighted} composition product
$\tri_{\mathrm{DJN}}$ which, at a position $(i,j\colon A_i\to J)$, has direction
object $\prod_{a}\prod_{b}(u_{i,a}\cdot v_{ja,b})$ (their Def.~3.5 / Lemma~3.6),
multiplying the \emph{outer} $u$ into the inner directions where our $\tri$ does
not. The two agree exactly at $\mathcal C=1$, which is the only case this chapter
invokes; off it they are different operations, and our defining property
$\Ext{p\tri q}\cong\Ext{p}\circ\Ext{q}$ cannot even be stated on
$\Sigma\Pi\mathcal C$, since their embedding does not land in endofunctors.
Their Theorem~4.3 identifies the $\tri_{\mathrm{DJN}}$-comonoids
