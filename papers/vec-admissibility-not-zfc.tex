...
\author{MacBeth}
\address{Kodamai}
\thanks{Internal technical note, Kodamai AI Mathematician program. Consolidates the PROVE sessions of 2026-09-20 and 2026-09-21. Comments to Neil Ghani and Robin Langer.}
...
\begin{remark}[An unverified sharpening]\label{rem:osofsky}
Osofsky's continuum-sensitive computation of the global dimension of $\End_k(E)$~\cite{osofsky} would, on the value $\mathrm{l.gl.dim}\,S=d+1$, sharpen the upper bound: as $M\subseteq\Phi$ is a submodule of a free module, $\pd_S M=\pd_S(\prod_{\N}N_0)-1\le\mathrm{gl.dim}\,S-1=d$, giving $\pd_S M=1$ exactly under CH. We flag this as resting on a recollection of Osofsky's exact value that we have \emph{not} verified at source; no result below depends on it, and the ZFC-clean bound of Proposition~\ref{prop:pd-bounds} ($\pd_S M\in\{1,2\}$ under CH) stands unconditionally.
\end{remark}
