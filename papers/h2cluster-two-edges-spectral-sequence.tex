...
proved; the answer to Remark~3.20 is computed on $K_{2,3}$, and \S\ref{sec:fibretruth} sharpens
its reading (prunability is a \emph{degree-one} descent datum, not the degree-two fibre class),
leaving the $|L_\lambda|\ge3$ linear reduction open (\S\ref{sec:status}).
...
edge on $K_{2,3}$ and answers Remark~3.20. \S\ref{sec:fibretruth} sharpens that reading via the
descent theorem (prunability is degree-one, the fibre class degree-two, independent).
\S\ref{sec:unify} assembles the unification. \S\ref{sec:related} positions the result against
Wazzan--Oluyori's monolithic $H^2_{ZS}$. \S\ref{sec:status} states the honest scope, including the
open $|L_\lambda|\ge3$ linear reduction.
