...
\emph{Author:} MacBeth (Kodamai).\qquad\emph{Date:} 9 September 2026.\\[2pt]
\emph{Corresponds to} \texttt{scotmacbeth/work-in-progress} \emph{commit} \texttt{99dd686}.\\[2pt]
\textbf{Draft --- work in progress; Kodamai grant deliverable.} This revision adds the
implementation-side ``parallel program'' motivation to \S\ref{sec:landscape}. A further landscape
item --- the additive-lens cartesian-closed structure over commutative monoids as prior art, with
this survey's linear-base obstructions as its negative complement --- is deferred pending
...
% TODO (write session 2026-09-09, BLOCKED on provenance): fold in the additive-lens CCC prior-art
% split here once sources are deep-read. Positive result (base Fam(CMon^op) is cartesian closed) is
% CHAD (Nunes-Vakar; arXiv id in scratch/write-2026-09-09.md) + Hedges' "Autodiff through function
% types"; DO NOT claim it. MY complement = the negative extension result (linear collapse Id^N) + the
% fullness gap = non-extensivity diagnosis. Deferred because CHAD is untracked in sources.json and the
% Hedges autodiff post is only at agent-summary; write sessions may not cite below deep-read and may
% not browse. This subsection currently makes NO additive-lens claim, so there is no overclaim to
% correct until that material is added. See scratch/write-2026-09-09.md and the for-robin note.
...
The map is worth drawing because the construction it maps keeps being redrawn. The
container/polynomial-functor account of a compositional construction is, with some regularity,
rediscovered from the implementation side by authors who do not cite --- and often do not know of
--- one another. Hedges' experimental language \emph{Poly} \cite{HedgesPoly}, whose types \emph{are}
containers and whose functions \emph{are} lenses, is a sharp recent instance: a working language
built directly on the Abbott--Altenkirch--Ghani \cite{AAG} and Gambino--Kock \cite{GambinoKock}
theory, its compiler targeting a graded monad modelling a stack --- over the classical base, but
arrived at from programming practice rather than from the categorical literature. When a construction
is independently reimplemented this often, a census of \emph{where} it has been run, and which of its
forms survives which base, stops being pedantry: it is what lets the reimplementers locate one
another, and it is what the rest of this section supplies.
...
\bibitem{HedgesPoly} J.~Hedges.
\emph{A first look at programming in Poly.}
Blog post, \texttt{julesh.com}, 29 June 2026. (An experimental language whose types are containers
and whose functions are lenses.)
