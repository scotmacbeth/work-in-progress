...
$\mathcal{K}(C,[\emptyset,X])\cong\mathcal{K}(\emptyset,X)\cong1$.) The argument
uses \emph{only} monoidal closedness and an initial object --- neither cartesian
closedness nor connectedness, the two independent axes.
\end{lemma}
...
extension $\sum_s[P_s,-]\colon\mathrm{Kl}(M)\to\mathrm{Kl}(M)$ is full and faithful
if and only if $M\cong\Id$. Equivalently $\mathcal{CL}\cap\mathcal{UN}=\{\Id\}$: no nontrivial
