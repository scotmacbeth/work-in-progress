...
The only monads left uncovered are analytic with $M\emptyset$ nonempty-finite \emph{and} unbounded degree
\emph{and} unbounded nesting depth --- a corner we argue is vacuous. The affine-plus-commutative folklore criterion is false ($\D$ is affine and
...
unbounded degree and unbounded nesting depth, a corner argued vacuous (Conjecture~\ref{conj:vacuous});
modulo that the theorem is an unconditional biconditional \textup(Theorem~\ref{thm:3final}\textup).
\end{theorem}
...
monads not covered (analytic, $M\emptyset$ nonempty-finite, unbounded degree and nesting depth) is
argued \emph{vacuous} in Conjecture~\ref{conj:vacuous}; that vacuity is the one statement we leave as a
conjecture, marked where it occurs. Full faithfulness for $\D$ rests on a standard rigidity fact,
...
right-cancellation that closes the analytic converse, leaving only a corner argued vacuous.
...
single corner we argue is vacuous (Conjecture~\ref{conj:vacuous}). The frame is classical: over $\Set$,
...
\end{enumerate}
The only monads not covered are analytic with $M\emptyset$ nonempty-finite, unbounded degree \emph{and}
unbounded nesting depth.
\end{theorem}

\begin{conjecture}[The uncovered corner is vacuous]\label{conj:vacuous}
For a finitary analytic monad ($A[n]$ finite for all $n$), $a_0=|M\emptyset|$ finite and unbounded
nesting depth are incompatible: unbounded nesting forces arbitrarily deep balanced iterated structures,
and a monad multiplication that builds these from a nonempty constant part substitutes closed terms
into the leaves, generating infinitely many closed structures ($a_0=\infty$) --- outside the finitary
hypothesis. Hence a finitary analytic monad with $a_0>0$ has bounded nesting depth and is covered by
case \textup{(c)}; the corner of Theorem~\ref{thm:dichotomy} is empty.
\end{conjecture}

We do not claim Conjecture~\ref{conj:vacuous} as proved; it converts the last gap from an opaque
plethysm question into a concrete tension between a nonempty finite constant part and deep free
structure, and it explains why no known monad inhabits the corner --- each candidate either drops its
constant part to $\emptyset$ (Theorem~\ref{thm:P}) or blows it up to $\infty$ (non-finitary, outside the
hypothesis). Assembling sufficiency with the trichotomy:
...
classes \textup{(a)--(c)} of Theorem~\ref{thm:dichotomy} --- hence every monad that arises --- has image
\emph{not} closed under composition. Modulo the vacuity of the residual corner
\textup{(}Conjecture~\ref{conj:vacuous}\textup{)}, ``$\extM{-}$ has composition-closed image $\iff$ $M$
is polynomial'' is an unconditional biconditional. The earlier reliance on an unproved
...
\item \textbf{The composition converse (the vacuous corner, Conjecture~\ref{conj:vacuous}).} Does
  ...
  analytic, $M\emptyset$ nonempty-finite, unbounded degree and nesting depth --- is argued vacuous in
  Conjecture~\ref{conj:vacuous}; proving that vacuity would make Theorem~\ref{thm:3final} an
  unconditional biconditional with no side hypothesis.
