...
arises, via a trichotomy: super-polynomial growth is killed by a cardinality law; every analytic monad
with $M\emptyset=\emptyset$ is killed by \emph{right-cancellation of plethysm} in the ring of
cycle-index series (with no bound on degree or nesting depth, so free magmas and free operad monads are
covered); and every bounded-degree analytic monad is killed by a support-splitting stabilizer invariant.
The only monads left uncovered are analytic with $M\emptyset$ nonempty-finite \emph{and} unbounded
nesting depth --- a corner that is \emph{inhabited} (the free commutative unital magma), where the
converse remains open. The affine-plus-commutative folklore criterion is false ($\D$ is affine and
...
Work-in-progress draft for Rick, 2026-09-07; work-in-progress commit 842db92.
\end{center}
...
\texttt{82e32d2}.\\[2pt]
\textbf{Draft --- work in progress; not yet neighbour-reviewed. This revision supersedes the version
sent to Rick on 5 September (commit \texttt{b94bc32}): Theorem~\ref{thm:3}'s necessity direction now
closes the analytic converse via plethysm right-cancellation (\S\ref{sec:cancel}), retiring the
previously open Plethysm Lemma.}
\end{minipage}}
\end{center}
...
polynomial --- proved for every monad that arises, by a trichotomy. Either $M$ grows
...
with $M\emptyset=\emptyset$, and \emph{right-cancellation of plethysm} in the cycle-index ring forces
$M$ polynomial with no bound on degree or nesting depth (Theorem~\ref{thm:P}, covering free magmas and
free operad monads); or $M$ is analytic of bounded degree, and a support-splitting stabilizer invariant
separates $M\circ M$ from $\ext{r}\circ M$ (Theorem~\ref{thm:S}, subsuming the multiset and
symmetric-power monads). The only monads uncovered are analytic with $M\emptyset$ nonempty-finite
and unbounded nesting depth, a corner that is \emph{inhabited} (Proposition~\ref{prop:corner}: the free
commutative unital magma); there the converse is open (Problem~\ref{prob:corner}), so outside this one
corner the theorem is a biconditional \textup(Theorem~\ref{thm:3final}\textup).
\end{theorem}
...
arises; \S\ref{sec:stab}--\ref{sec:cancel}), and then AAG strong monoidality delivers the splice
$p\triangleleft\ceil M\triangleleft q$. The trailing $M$ cannot be cancelled at the level of
connected-limit preservation --- Proposition~\ref{prop:inert} shows the single self-composite
$M\circ M\cong\ext r\circ M$ carries no such information beyond the retract reduction
Lemma~\ref{lem:R0}. The converse therefore uses a \emph{finer, faithful} invariant: an
$\mathrm{Aut}(X)$-stabilizer statistic (Lemma~\ref{lem:stab}) for bounded degree, and, for analytic
$M$ with $M\emptyset=\emptyset$, right-cancellation of plethysm on the cycle-index series
(Lemma~\ref{lem:cancel}) --- a genuine cancellation of the trailing $M$, now on the level of species
isomorphism rather than limit preservation, reaching the unbounded-nesting monads no stabilizer count
can touch.
...
direction for every monad that arises: the cardinality law disposes of the super-polynomial monads;
plethysm right-cancellation (Lemma~\ref{lem:cancel}, Theorem~\ref{thm:P}) disposes of every analytic
monad with $M\emptyset=\emptyset$, degree and nesting depth unbounded; and the support-splitting
stabilizer invariant (Lemma~\ref{lem:stab}, Theorems~\ref{thm:multiset} and \ref{thm:S}) disposes of
every bounded-degree analytic monad. The cancellation engine (Lemma~\ref{lem:cancel}) is elementary and
computationally verified; Theorems~\ref{thm:P} and \ref{thm:S} additionally invoke two standard facts of
combinatorial-species theory --- Joyal's full faithfulness of the species-to-analytic-functor embedding,
and the identity of species substitution with plethysm of cycle-index series (Joyal;
Bergeron--Labelle--Leroux) --- which we name as background rather than re-derive. The single class of
monads not covered (analytic, $M\emptyset$ nonempty-finite, unbounded nesting depth) is \emph{inhabited}
--- by the free commutative unital magma (Proposition~\ref{prop:corner}) --- and the converse there is
genuinely open (Problem~\ref{prob:corner}); this is the one gap in the necessity direction, marked where
it occurs. Full faithfulness for $\D$ rests on a standard rigidity fact,
...
Theorem~\ref{thm:3}: sufficiency, the cardinality law, the stabilizer invariant, and the plethysm
right-cancellation that closes the analytic converse for $M\emptyset=\emptyset$, leaving one inhabited
corner open.
...
We prove Lemma~\ref{lem:N} for every monad that arises in practice, reducing the sole residual to a
single corner that is inhabited (Proposition~\ref{prop:corner}) and open there (Problem~\ref{prob:corner}). The frame is classical: over $\Set$,
...
The multiset argument reaches only \emph{bounded} nesting: it pits one wreath $H\wr S_r$ against Young
subgroups. For monads whose structures nest to unbounded depth --- the free (commutative) magma, free
operad-algebra monads, with automorphism towers $S_2\wr\cdots\wr S_2$ --- no fixed stabilizer count
suffices, and we replace the combinatorics with one algebraic identity.

\subsection{Cycle-index cancellation, and the necessity trichotomy}\label{sec:cancel}
Recall the analytic/species dictionary (Joyal; Bergeron--Labelle--Leroux). An \emph{analytic} functor
is one of the form $\tilde A(X)=\sum_{n\ge0}A[n]\times_{S_n}X^{n}$ for a \emph{species} $A$ (each $A[n]$
a finite $S_n$-set); its \emph{cycle-index series} is
\[
...
\]
Three standard facts. \textbf{(J1)} $A\mapsto\tilde A$ is fully faithful: $\tilde A\cong\tilde B$ iff
$A[n]\cong B[n]$ as $S_n$-sets for all $n$. \textbf{(F)} $\tilde A$ is polynomial iff its species is
\emph{flat} (every $S_n$-action on $A[n]$ free), iff $Z_A\in\mathbb Q[[p_1]]$ (only $p_1$-powers: a
non-identity permutation fixing a structure contributes a $p_\lambda$ with $\lambda\ne(1^n)$).
\textbf{(C)} if $B[0]=\emptyset$ then species substitution is plethysm on cycle indices,
$Z_{A\bullet B}=Z_A\circ Z_B$, where $\circ$ is the unique $\mathbb Q$-algebra endomorphism of $R$ with
$p_k\circ H=H[p_i\mapsto p_{ki}]$. Writing $a_0=|A[0]|=|M\emptyset|$ and $a_1=|A[1]|$, a monad has
$a_1\ge1$ (the unit $\Id\Rightarrow\tilde A$ requires an $S_1$-fixed point of $A[1]$).
...
For $M\emptyset\ne\emptyset$ the self-composite is non-finitary and its cycle index diverges; the
bounded-degree case is recovered by a sharpening of the stabilizer invariant.

\begin{theorem}[Support-splitting converse; bounded degree]\label{thm:S}
Let $M=\tilde A$ be a bounded-degree analytic monad ($e_{\max}:=\sup_l\deg<\infty$; equivalently
$\tilde A$ finitary), with any $M\emptyset$. If $A$ is non-flat then $\tilde A\circ\tilde A\ncong\ext
r\circ\tilde A$ for every polynomial $\ext r$; hence closure forces $M$ polynomial.
\end{theorem}

\begin{proof}[Proof sketch]
For $K\le\mathrm{Sym}(\Omega)$ the support-splittings ($\mathrm{supp}(K)=\bigsqcup_t C_t$ with
$K=\prod_t K_t$, $K_t=\{k\in K:\mathrm{supp}(k)\subseteq C_t\}$) are closed under common refinement, so
there is a unique finest one; the multiset of \emph{factor degrees} $\{|C_t|\}$ is a conjugacy
invariant (this refines Lemma~\ref{lem:stab}). With $\ext r=\tilde B$, $B$ flat and bounded, every
constituent of $B\bullet A$ is a Young product of $A$-stabilizers, whose support-indecomposable factors
have degree $\le e_{\max}$. But a non-flat constituent $X^d/H$ ($H\ne1$), fed the constant colouring by
a maximal-degree constituent $X^{e_{\max}}/H_c$, yields in $A\bullet A$ a wreath $K=(H_c)^d\rtimes H$ in
which some $h\in H$ links two size-$e_{\max}$ blocks, producing a support-indecomposable factor of
degree $\ge2e_{\max}$. The invariant separates: $A\bullet A\ncong B\bullet A$. This subsumes
Theorem~\ref{thm:multiset} (for $\mathbb M$, $e_{\max}$ was there avoided by hand).
\end{proof}

Assembling the three obstructions:

\begin{theorem}[Necessity, trichotomy]\label{thm:dichotomy}
Let $M$ be a non-polynomial monad on $\Set$. The essential image of $\extM{-}$ is not closed under
composition whenever $M$ is
\begin{enumerate}[label=\textup{(\alph*)},leftmargin=2.2em]
\item \emph{super-polynomially growing} --- excluded by the cardinality law
  (Proposition~\ref{prop:cardlaw}: powerset, nonempty powerset, distributions, ultrafilter,
  continuation, filter); or
\item \emph{analytic with $M\emptyset=\emptyset$} --- excluded by plethysm right-cancellation
  (Theorem~\ref{thm:P}), at any degree and any nesting depth: free magmas, free operad-algebra monads;
  or
\item \emph{analytic of bounded degree}, any $M\emptyset$ --- excluded by the support-splitting
  invariant (Theorem~\ref{thm:S}): the multiset and symmetric-power monads, every bounded-arity
  finite-signature monad.
\end{enumerate}
The only monads not covered are analytic with $M\emptyset$ nonempty-finite and unbounded nesting depth
(which forces unbounded degree). This corner is \emph{inhabited} (Proposition~\ref{prop:corner}), so the
residual is precisely $\{\,a_0>0\ \text{and unbounded nesting depth}\,\}$.
\end{theorem}

\begin{proposition}[The uncovered corner is inhabited]\label{prop:corner}
The residual corner of Theorem~\ref{thm:dichotomy} is \emph{nonempty}. The free commutative unital
\textup{(}non-associative\textup{)} magma monad $U$ --- the free-algebra monad of the theory with one
nullary operation $c$ \textup{(}a unit\textup{)} and one commutative binary operation $\mu$ subject to
$\mu(x,c)=x$ --- is a finitary analytic monad lying in it. Its species is
\[
  U[0]=\{c\}\ (a_0=1),\qquad U[n]=\{\text{commutative binary trees on $n$ leaves}\},\quad
  |U[n]|=(2n-3)!!\ (n\ge1),
\]
so every component is finite; the balanced $2^k$-leaf tree has stabilizer $S_2\wr\cdots\wr S_2$ \textup{(}$k$
factors\textup{)}, giving unbounded nesting depth --- hence unbounded degree, since a depth-$k$ tower needs
arity $\ge2^k$ --- together with $a_0=1$ nonempty-finite.
\end{proposition}

This refutes the earlier hope that the corner is vacuous. That vacuity sketch assumed a nonempty
constant part, substituted into deep structure, \emph{builds} new closed deep terms and so forces
$a_0=\infty$; but a \emph{unit} \emph{absorbs} rather than builds ($\mu(x,c)=x$). Adjoining a unit to the
free commutative magma --- which has $a_0=0$ and is already covered by Theorem~\ref{thm:P} --- raises
$a_0$ from $0$ to $1$ while leaving every positive-arity constituent, and hence the entire
$S_2\wr\cdots\wr S_2$ nesting tower, untouched. So $a_0$ stays finite $(=1)$ with nesting depth still
unbounded. The lesson is that the true axis of this corner is \emph{nesting \textup{(}wreath\textup{)}
depth}, not degree: bounded degree implies bounded nesting depth, so a depth-based refinement of
Theorem~\ref{thm:S} --- a conjectural bounded-nesting-depth invariant $\mathrm S'$ subsuming
Theorem~\ref{thm:S} --- would collapse the residual to exactly $a_0>0$ with unbounded nesting depth, of
which $U$ is the canonical inhabitant. What remains genuinely open is the fate of $U$ itself.

\begin{problem}[The sharp residual corner]\label{prob:corner}
Decide whether $U\circ U\cong\ext r\circ U$ for some polynomial container $\ext r$. A \emph{negative}
answer closes the analytic necessity of Theorem~\ref{thm:3} as a biconditional over every finitary
analytic monad, with $U$ the extremal case. A \emph{positive} answer would exhibit a non-polynomial
\textup{(}non-flat, hence not preserving connected limits\textup{)} monad whose essential image is
nonetheless closed under composition --- refuting the necessity direction inside this corner. The
question is open: neither plethysm right-cancellation \textup{(}which needs $a_0=0$\textup{)} nor the
support-splitting degree gap \textup{(}which needs bounded degree, hence bounded nesting depth\textup{)}
reaches it.
\end{problem}

Assembling sufficiency with the trichotomy:
...
\textup{(}Proposition~\ref{prop:suff}\textup{)}. \emph{Necessity:} every non-polynomial monad in
classes \textup{(a)--(c)} of Theorem~\ref{thm:dichotomy} --- hence every monad that arises in practice
--- has image \emph{not} closed under composition. The sole exception is the corner $\{a_0>0$, unbounded
nesting depth$\}$, which is \emph{inhabited} \textup{(}Proposition~\ref{prop:corner}, the free
commutative unital magma $U$\textup{)}; there necessity is open \textup{(}Problem~\ref{prob:corner}\textup{)}.
Thus ``$\extM{-}$ has composition-closed image $\iff$ $M$ is polynomial'' is a proved sufficiency
together with a biconditional for every monad outside that one corner. The earlier reliance on an unproved
species-theoretic \emph{Plethysm Lemma} is removed: for $M\emptyset=\emptyset$ the required inequality
$A\bullet A\ncong B\bullet A$ follows directly from right-cancellation in the cycle-index ring,
bypassing any identification of a ``bad'' constituent.
\end{theorem}
...
\item \textbf{The composition converse (the inhabited corner, Problem~\ref{prob:corner}).} Does
  closure of the essential image under $\circ$ force $M$ to be polynomial? Yes for every monad that
  arises (Theorem~\ref{thm:dichotomy}): the cardinality law excludes the super-polynomial monads,
  plethysm right-cancellation (Lemma~\ref{lem:cancel}) excludes every analytic monad with
  $M\emptyset=\emptyset$ at any nesting depth, and the support-splitting invariant excludes every
  bounded-degree analytic monad. The trailing-$M$ obstruction was sharpened into the exact ``no
  information'' statement of Proposition~\ref{prop:inert} at the level of limit preservation, then
  circumvented by cancelling on the finer cycle-index invariant. The one class left uncovered ---
  analytic, $M\emptyset$ nonempty-finite, unbounded nesting depth --- is \emph{inhabited} by the free
  commutative unital magma $U$ (Proposition~\ref{prop:corner}); the true axis there is nesting depth, not
  degree. Deciding whether $U\circ U\cong\ext r\circ U$ (Problem~\ref{prob:corner}) would either complete
  the biconditional or exhibit a non-polynomial counterexample with composition-closed image.
