...
and the outer peg $E'$. (A second, non-absorbed nullary would sit as a rigid single-element peg. As a uniquely
labelled rigid leaf it is fixed pointwise by every automorphism of any tree in which it occurs, so it
contributes only a trivial $\times1$ direct factor to every stabilizer; the argument survives, but we do
not need that generality for the corollary.) Theorem~\ref{thm:main} itself places no bound on $a_0=|M\emptyset|$:
