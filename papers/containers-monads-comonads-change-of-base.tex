...
\emph{Corresponds to} \texttt{scotmacbeth/work-in-progress} \emph{content-commit} \texttt{129fecd}.\\[2pt]
...
\emph{Step 2: $\CL$ forces $E\cong1$.} Let $M\in\CL\cap\UN$, so $MX\cong X\times E$. Monoidal closure
requires $(-)\times A$ to have a right adjoint for every object $A$, in particular for $A=\emptyset$.
For all $C$,
\[
  \Kl(M)(C\times\emptyset,\,B)=\Kl(M)(\emptyset,B)=\Set(\emptyset,MB)=1,
\]
so the representing object $[\emptyset,B]$ is terminal in $\Kl(M)$; call it $\top$. A terminal object
