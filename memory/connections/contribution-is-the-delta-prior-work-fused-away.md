...
| **Generalized poly (DJN 2305.05655)** | Dorta–Jarvis–Niu: ⊗ (= mine) and the **weighted `◁_DJN`** (≠ mine off `C=1`) over a general monoidal base; Thm 4.3 comonoids-in-`◁_DJN`=enriched cats — ⚠ **2026-08-31 `speculative`: whether this subsumes my Vec-algebroid is now OPEN**, the operations differ and the classifications differ (families of k-algebras vs all enriched cats); not a restored novelty claim | **omit** closedness + the extensivity/fullness question + Set→Vec change-of-enrichment | exactly those three (T1 proved, T2 open, T3 proved) → [[reference_dorta_jarvis_niu_generalized_poly]] |
